...
#
# V7.1 — CHANGEMENTS PAR RAPPORT A V7 :
#   - Percentiles de revente abaissés (0.25→0.10 et 0.40→0.25)
#     pour compenser le biais de survie : les annonces actives
#     sur Vinted sont surtout celles qui NE SE SONT PAS vendues
#     (donc souvent trop chères). Se baser sur les moins chers
#     du marché donne une estimation plus proche d'un prix
#     réellement atteignable.
#   - Détection des lots Funko Pop avec un prix à l'unité
#     indiqué en description (ex: "5€ pièce", "3€ chacun").
#     Ces annonces affichent un prix d'appel qui n'est pas le
#     vrai coût total du lot : elles sont maintenant exclues.
#
# ============================================================
...
# --------------------------------------------------------------
# PRIX A L'UNITE EN DESCRIPTION
# --------------------------------------------------------------
#
# Certains vendeurs affichent un prix d'appel bas (1€, 5€) sur
# le lot, mais précisent en description que ce prix s'applique
# À CHAQUE Funko individuellement ("5€ pièce", "3€ chacun",
# "l'unité 4€"...). Le vrai coût total du lot est alors bien
# plus élevé que le prix affiché. Ces annonces faussent le
# calcul de marge et doivent être exclues.
#
PER_UNIT_PRICE_RE = re.compile(
    r"\b\d+(?:[.,]\d+)?\s*€?\s*"
    r"(?:/|par|chacun|chacune|piece|pièce|"
    r"l'unite|l'unité|unite|unité|each|"
    r"par piece|par pièce)\b"
    r"|"
    r"\b(?:piece|pièce|unite|unité|each)\s*"
    r"[:=]?\s*\d+(?:[.,]\d+)?\s*€"
)


def has_per_unit_pricing(description):
...
def analyse_item(
    ...
    if category == "funko":
        ...
        if has_per_unit_pricing(
            ...
            record_rejection(
                "Funko : prix à l'unité indiqué en "
                "description (pas un vrai prix de lot)",
                item,
            )
    ...
    # ------------------------------------------------------
    # NOTE V7.1 : percentiles abaissés (0.25→0.10, 0.40→0.25)
    # pour compenser le biais de survie des annonces actives
    # sur Vinted (les articles vendus disparaissent, ceux qui
    # restent sont souvent surévalués).
    # ------------------------------------------------------
...
def send_email(
    ...
    html_body += (
        ...
        "Bitty Pop, produits similaires, "
        "et des annonces avec prix à l'unité "
...
def main():
    ...
    print(
        ...
        "(hors annonces avec prix à l'unité en description)"
    )
    ...
    print(
        "Pas de lots Funko avec prix à l'unité en description."
    )
