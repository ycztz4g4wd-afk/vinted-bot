...
# PISTE 18 : rotation des mots-clés par lots. Avec 43 mots-clés x 6 pays et
# les pauses anti-détection, un scan complet peut désormais prendre 20-40+
# min alors que le cron tourne toutes les 15 min -> risque de runs qui se
# chevauchent ou de timeout GitHub Actions. Plutôt que de tout scanner à
# chaque run, on scanne 1/3 des mots-clés à chaque fois, en tournant sur un
# index basé sur le temps (pas besoin de fichier d'état persistant : le
# calcul est déterministe et avance tout seul avec l'horloge). Sur 45 min
# (3 cycles de 15 min), TOUS les mots-clés finissent scannés, mais chaque
# run individuel est ~3x plus rapide et fait ~3x moins d'appels API
# (moins de risque de rate-limit/ban).
KEYWORD_BATCH_COUNT = 3
CRON_INTERVAL_SECONDS = 15 * 60


def get_keyword_batch():
    idx = int(time.time() // CRON_INTERVAL_SECONDS) % KEYWORD_BATCH_COUNT
    # keywords[idx::N] répartit les catégories entre les lots au lieu de
    # découper en blocs contigus (évite qu'un lot entier n'ait que des
    # mots-clés Nike par ex.)
    batch = KEYWORDS[idx::KEYWORD_BATCH_COUNT]
    random.shuffle(batch)  # équité entre les mots-clés du même lot
    return batch


# PISTE 20 : rotation du tri de recherche. En triant TOUJOURS par "plus
# récent" (newest_first), le bot ne peut structurellement jamais tomber sur
# une annonce bon marché mais ancienne, publiée il y a des semaines et
# jamais vendue faute d'avoir été repérée - exactement le genre de pépite
# oubliée qu'on cherche. Un run sur 4 (~1h) trie par prix croissant à la
# place, pour aller chercher ce angle mort sans coût API supplémentaire
# (toujours per_page=10).
ORDER_ROTATION_CYCLES = 4


def get_search_order():
    idx = int(time.time() // CRON_INTERVAL_SECONDS) % ORDER_ROTATION_CYCLES
    return "price_low_to_high" if idx == 0 else "newest_first"
...
# PISTE 14 : BUGFIX état de l'article. status_id/condition_id sont TOUJOURS
# None dans les réponses Vinted (vérifié sur des centaines d'articles dans
# les logs) -> le filtre d'état n'a jamais rejeté un seul article depuis le
# début, et l'affichage montrait "État non précisé" même quand Vinted
# donnait l'info. Le vrai état est dans le champ TEXTE 'status'
# ("Très bon état", "Sehr gut", "Buone", "Very good"...), déjà localisé par
# Vinted -> on l'utilise directement, pas besoin de mapping numérique.
POOR_CONDITION_KEYWORDS = ["satisf", "zufriedenstellend", "soddisf"]  # FR/EN/ES vs DE vs IT


def is_poor_condition(status_text):
    if not status_text:
        return False
    t = status_text.casefold()
    return any(kw in t for kw in POOR_CONDITION_KEYWORDS)
...
# PISTE 19 : filtre size_ids AU NIVEAU DE L'API. Le rejet le plus fréquent
# de très loin dans tous nos logs est "taille non retenue" (souvent 70-80%
# des articles scannés) -> on récupère plein d'articles pour rien avant de
# les jeter. L'API Vinted accepte un paramètre size_ids[] pour ne renvoyer
# QUE les tailles demandées dès la recherche. IDs vérifiés via le dataset
# public teddy-vltn/vinted-dataset (utilisé aussi par le moniteur Vintrack) :
# groupe "4" = vêtements adulte lettres (S=3, M=4, L=5, XL=6, XXL=7),
# groupe "7" = chaussures EU générique (41=61, 42=62, 43=63).
# Ne couvre pas 100% des sous-catégories obscures (ceintures, chapeaux ont
# leurs propres groupes de tailles) -> on garde size_is_accepted() en filet
# de sécurité derrière, mais ça élimine la grosse majorité du bruit taille
# enfant/hors gabarit dès la requête, donc chaque page de résultats est
# beaucoup plus utile.
SIZE_IDS_FILTER = [3, 4, 5, 6, 7, 61, 62, 63]
...
# PISTE 12 : la recherche Vinted sur "lot Nike"/"lot Carhartt" etc. renvoie
# aussi plein d'articles UNIQUES qui matchent juste vaguement le mot-clé
# (Vinted ne fait pas une recherche stricte du mot "lot" dans le titre).
# Sans vérification, ces articles uniques finissaient dans la section
# "lots" sans jamais être évalués -> on vérifie que le titre contient
# vraiment un indicateur de groupement avant de le traiter comme un lot ;
# sinon il repart dans le pipeline normal (taille + marge) comme un article classique.
BUNDLE_INDICATOR_RE = re.compile(
    r"\b(lots?|bundle|pack(?:\s+de|\s+of)?|paquet)\b"
    r"|\b\d+\s*(?:x\b|polos?|t-?shirts?|tee\s*shirts?|pulls?|paires?|"
    r"chaussures?|shorts?|maillots?|v[eê]tements?|pi[eè]ces?|items?)\b"
    r"|\bx\s*\d+\b",
    re.IGNORECASE,
)
...
def looks_like_actual_bundle(title):
    return bool(BUNDLE_INDICATOR_RE.search(title or ""))


# PISTE 13 : filtre vêtements enfant. Le filtre de taille normal (S/M/L/XL/
# XXL + pointures 41-43) élimine déjà la plupart des tailles enfant, MAIS
# les lots groupés sautent volontairement ce filtre (un lot n'a pas une
# taille unique) -> des lots bébé/enfant s'y glissaient ("baby girl 6
# months", "Lot 5/6 ans fille"...). On vérifie explicitement le titre (et
# la taille si disponible) pour ne garder que du homme/femme adulte.
KIDS_INDICATOR_RE = re.compile(
    r"\b(\d+\s*(?:ans?|ann[ée]es?|years?|yrs?|mois|months?|jahre?|jahr|anni|anno|años|meses|mes)\b"
    r"|enfant|b[ée]b[ée]s?|babys?|babies|kids?|gar[cç]ons?|filles?|boys?|girls?|junior|toddler|"
    r"infant|child(?:ren)?|bambin[oa]?|ni[ñn][oa]s?|kind(?:er)?)\b",
    re.IGNORECASE,
)
...
def is_hype_item(item):
    ...
    if any(b.casefold() in title for b in HYPE_BRANDS):
        return True
    # PISTE 15 : certains MODÈLES précis souffrent du même biais de prix
    # aspirationnels même sous une marque "normale" (Nike, New Balance) qui
    # elle-même n'est pas hype dans l'ensemble de son catalogue - vu en
    # pratique : Nike Dunk Low / Air Jordan et New Balance 2002R affichent
    # des médianes de 100-120€ très suspectes, comme Supreme.
    return any(m in title for m in HYPE_MODEL_KEYWORDS)


HYPE_MODEL_KEYWORDS = {"dunk low", "air jordan", "jordan 1", "jordan 4", "2002r", "travis scott"}
...
def search_vinted(s, domain, kw):
    # PISTE 19 (suite) : pas de filtre size_ids pour les recherches de lots
    # -> un lot est souvent tagué "Taille unique" côté Vinted, pas une
    # taille S-XXL/41-43, donc filtrer dessus exclurait les vrais lots.
    params = {
        "search_text": kw,
        "price_to": PRICE_MAX,
        "price_from": PRICE_MIN,
        "order": get_search_order(),
        "per_page": 10,  # réduit de 40 à 10 pour limiter le volume de requêtes
        "currency": "EUR",
    }
    if kw not in LOT_KEYWORDS:
...
def send_email(deals, bundles=None):
    ...
    # PISTE 17 : trier par fraîcheur (peu de vues/favoris) plutôt que dans
    # l'ordre brut du scan. Un article sous-évalué avec 0 vue/favori =
    # personne ne l'a encore repéré, meilleure chance qu'il soit encore
    # disponible. Un article déjà à 15 favoris est probablement déjà
    # "découvert" par d'autres chasseurs de deals.
    def engagement(d):
        item = d[0]
        return (item.get("view_count") or 0) + (item.get("favourite_count") or 0)

    reliable.sort(key=engagement)
    review.sort(key=engagement)
    ...
    def render_section(title, items, note):
        ...
        for d in items:
            ...
            favs = item.get("favourite_count")
            if views is not None or favs is not None:
                html += f"👀 {views or 0} vues | ❤️ {favs or 0} favoris<br>"
    ...
    def render_bundles(items):
        ...
        sorted_items = sorted(items, key=lambda b: (b[0].get("view_count") or 0) + (b[0].get("favourite_count") or 0))
        for item, price, state_label, domain in sorted_items:
            item_id = item.get("id")
            link = item.get("url") or (f"https://www.{domain}{item.get('path')}" if item.get("path") else "") or (f"https://www.{domain}/items/{item_id}" if item_id else "")
            html += "<div style='border:1px solid #999;padding:10px;margin:10px 0'>"
            html += f"<b>[{domain}] " + item.get("title", "") + "</b><br>"
            ...
            if views is not None or favs is not None:
                html += f"👀 {views or 0} vues | ❤️ {favs or 0} favoris<br>"
            html += f"Prix du lot: {price}€ | État: {state_label}<br>"
            if link:
                html += f'<a href="{link}">Voir l\'annonce sur Vinted</a>'
            html += "</div>"
        return html
...
def scan_domain(domain, seen):
    ...
    keyword_batch = get_keyword_batch()
    print(f"[{domain}] Lot de mots-clés ce run : {len(keyword_batch)}/{len(KEYWORDS)}")

    for kw in keyword_batch:
        results = search_vinted(s, domain, kw)
        # PISTE 11 : détection d'un blocage silencieux. Une recherche vide
        # de temps en temps est normal (mot-clé de niche, "lot X" rare...),
        # mais plusieurs mots-clés d'affilée à 0 résultat sur un domaine qui
        # tournait bien juste avant est un signal classique de rate-limit/
        # ban IP silencieux (status 200 mais résultats vidés côté anti-bot),
        # pas une vraie pénurie d'annonces.
        if not results and kw not in LOT_KEYWORDS:
            consecutive_empty += 1
            if consecutive_empty == 5:
                print(f"[{domain}] ⚠️ ALERTE: 5 mots-clés d'affilée à 0 résultat — possible blocage/rate-limit silencieux plutôt qu'une vraie pénurie d'annonces.")
        else:
            consecutive_empty = 0
        for itm in results:
            iid = str(itm.get("id"))
    ...
    for itm in items:
        ...
        status_text = itm.get("status") or ""
        if is_poor_condition(status_text):
            print(f"[{domain}] REJET '{title}': état trop faible ({status_text})")
            continue
        ...
        if looks_like_kids_item(full_title, itm.get("size_title")):
            print(f"[{domain}] REJET '{title}': vêtement enfant détecté")
            continue

        is_lot = itm.get("_search_kw") in LOT_KEYWORDS and looks_like_actual_bundle(full_title)
        if is_lot:
            ...
            state_label = status_text or "État non précisé par Vinted - vérifie sur l'annonce"
            print(f"[{domain}] LOT '{title}': prix {price}€, état={state_label} | url={itm.get('url')!r} path={itm.get('path')!r} id={itm.get('id')}")
            bundles.append((itm, price, state_label, domain))
            pause_courte()
            continue
        ...
        if not mp:
            ...
            pause_courte()
            continue
        # PISTE 16 : Vinted fournit le vrai montant des frais de protection
        # acheteur dans 'service_fee' -> plus précis que notre estimation
        # (BUYER_PROTECTION_PERCENT/FIXED), qui peut dériver si Vinted change
        # sa grille tarifaire. On utilise le vrai chiffre quand il est là,
        # notre estimation seulement en repli si le champ est absent.
        real_fee = (itm.get("service_fee") or {}).get("amount")
        pf = float(real_fee) if real_fee else round(price * BUYER_PROTECTION_PERCENT + BUYER_PROTECTION_FIXED, 2)
