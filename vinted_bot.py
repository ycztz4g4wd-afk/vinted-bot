...
# Prix de vente conseille : un peu en dessous du prix median pour vendre plus vite
SUGGESTED_SALE_RATIO = 0.85
...
BUYER_PROTECTION_PERCENT = 0.05     # approximation de la formule Vinted (~5%)
BUYER_PROTECTION_FIXED = 0.70       # partie fixe approximative
...
def estimate_buyer_protection(price):
    """Estimation approximative des frais de protection acheteur Vinted."""
...
def send_email(deals, others):
    ...
    if deals:
        ...
        for item, market_price, comparables_count, total_cost, net_profit, suggested_price, suggested_profit in deals:
            badge = (
                ...
                f"Prix de vente conseille (pour vendre rapidement) : {suggested_price} EUR<br>"
                f"Marge nette estimee a ce prix : ~{suggested_profit} EUR"
                f"</div>"
            )
...
def main():
    ...
    for item in new_items_all:
        ...
        if market_price is not None:
            ...
            if is_deal:
                deals.append((item, market_price, comparables_count, total_cost, net_profit, suggested_price, suggested_profit))
            else:
                others.append(item)
        else:
            others.append(item)
