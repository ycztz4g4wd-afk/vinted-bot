...
#   - basket non-NBA
#
# Le classement est volontairement beaucoup plus strict.
#
# ============================================================
...
# Nombre de recherches d'équipes par domaine à chaque run.
#
# Les recherches sont volontairement réparties dans le temps.
# Cela évite d'envoyer 100+ requêtes par domaine à chaque run.
TEAM_SEARCHES_PER_DOMAIN = 20

# Rotation des recherches toutes les X minutes.
SEARCH_ROTATION_MINUTES = 60

# Nombre maximum d'opportunités dans un email.
MAX_EMAIL_DEALS = 25

# Nombre maximum d'alertes Discord par run.
...
# Pour les maillots, on ne cherche PAS de chaussures.
...
# IMPORTANT :
#
# "Replica jersey" n'est PAS automatiquement refusé.
#
# Dans le football, une "replica" peut désigner une version
# officielle destinée aux supporters.
#
# On refuse surtout les termes réellement suspects.
# ============================================================
...
def is_football_item(
    ...
    Il faut obligatoirement :
        ...
        1. un signal vintage
        2. une équipe européenne connue
        3. un terme indiquant un vêtement / maillot

    Donc :

        "Ralph Lauren rose shirt vintage"
        -> REFUSE

        "NFL vintage jersey"
        -> REFUSE

        "Argentina vintage jersey"
        -> REFUSE

        "Real Madrid vintage jersey"
        -> ACCEPTE

        "Manchester United 1998 shirt"
        -> ACCEPTE
    """
    ...
    # NFL / NBA / MLB / NHL = pas football européen.
    if contains_any_phrase(
        text,
        NON_FOOTBALL_SPORTS,
    ):
    ...
    # Equipe européenne connue obligatoire.
    if not has_known_european_team(
        text
    ):
        return False

    # Vêtement obligatoire.
    if not has_football_garment(
        text
    ):
        return False
...
def classify_sport(
    ...
    # Si les deux sont détectés,
    # on préfère refuser pour éviter une mauvaise catégorie.
    return None
...
def build_team_search_plan():
    ...
    # On alterne foot / NBA pour que chaque rotation
    # contienne les deux catégories.
    max_length = max(
        len(football),
        len(nba),
    )
...
def select_search_plan(
    ...
    # --------------------------------------------------------
    # RECHERCHES GENERIQUES TOUJOURS PRESENTES
    # --------------------------------------------------------
...
def normalize_photo_identifier(
    ...
    try:
        ...
        # On garde principalement le chemin.
        # Cela permet de reconnaître une même photo
        # même si le domaine CDN change.
        path = parsed.path
...
# ============================================================
#
# C'est ici qu'on règle une grosse partie des doublons.
#
# L'ID Vinted seul ne suffit pas :
# un même article peut être retrouvé sur plusieurs domaines.
#
# On utilise donc :
#
#   - photo
...
#   - taille
#
# Le prix n'est volontairement PAS inclus :
# si le vendeur baisse le prix, le bot reconnaît toujours
# le même article.
# ============================================================
...
def get_stable_fingerprint(
    ...
    # Photo disponible mais vendeur absent.
    elif photo:
...
def comparable_matches_target(
    ...
    # Etat :
    #
    # Si Vinted fournit l'état du comparable,
    # on refuse les mauvais états.
    #
    ...
    if (
        comparable_condition
        and not is_accepted_condition(
            comparable
        )
    ):
        return False

    target_text = get_item_search_text(
        target
    )

    comparable_text = get_item_search_text(
        comparable
    )

    # --------------------------------------------------------
    # FOOTBALL :
    # même équipe si l'équipe est détectable.
    # --------------------------------------------------------
...
def get_comparables(
    ...
    # --------------------------------------------------------
    # Recherche comparable spécialisée.
    # --------------------------------------------------------
    ...
    for comparable in data.get(
        ...
        # On évite les comparables complètement aberrants.
        if price > 100:
            continue
...
def analyse_item(
    ...
    # --------------------------------------------------------
    # LIMITE PAR RAPPORT AU BUDGET
    # --------------------------------------------------------
    #
    # Avec 20 € de budget, il n'est pas logique de calculer
    # les comparables de tous les articles à 30-40 €.
    #
    # On autorise exceptionnellement jusqu'à 65 % du budget.
    # --------------------------------------------------------

    max_analysis_price = (
        budget
        * MAX_BUDGET_USAGE_EXCEPTIONAL
    )

    if purchase_price > max_analysis_price:
        return None
    ...
    # Prix anormalement faible par rapport aux comparables.
    if (
        purchase_price < 3
    ...
    # Marques hype non directement liées aux maillots.
    if (
        is_hype_item(item)
        and purchase_price
...
def should_notify_deal(
    ...
    # --------------------------------------------------------
    # RE-NOTIFICATION APRES 30 JOURS
    # --------------------------------------------------------
...
def main():
    ...
    # --------------------------------------------------------
    # RETIRE LES DOUBLONS DEJA ENVOYES
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # SI EMAIL ENVOYE :
    # ON MARQUE LES ARTICLES
    #
    # C'est cette partie qui empêche les doubles mails.
    # --------------------------------------------------------
